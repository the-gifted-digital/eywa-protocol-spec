...
- `sitemap_section` `text` — **site zone as a NUMBER, `'1'`..`'9'`**, for all three brands (deezy-dental's slug form was migrated 2026-08-23). *(corrected 2026-08-24 against live schema — "operator-defined section grouping" understated it. Live distribution: 3=630, 6=695, 5=393, 8=196, 7=135, 9=126, 4=116, 2=62, 1=5; no NULLs. *(re-measured 2026-08-24 after vth-biodent moved 93 leaf pages into the zone their `page_category` names — §3 658→630, §6 617→695, §5 444→393. slug was untouched; the fingerprint↔node_id invariant means a zone change is a renumbering, not a URL change.)*)* This is a ZONE, never a substitute for `page_category` when the rule asks what a page **is** — section 5 on smile-scape-clinic holds 91 `condition_pillar` rows plus 80 `service_page` and five other kinds.
...
- `page_category` `text` 🆕 *(added 2026-08-24 — live column, never documented)* — **SEMANTIC PAGE KIND**, the replacement for the category half of `page_type`. Controlled values: `home`, `about`, `contact`, `condition_pillar`, `procedure_pillar`, `service_page`, `technology_page`, `knowledge_article`, `evidence_case`, `doctor_profile`, `branch_landing`, `local_landing`, **`pricing_page`**, **`insurance_page`** — plus `local_programmatic` and `local_service_page`, which occur in the data. *(`pricing_page` added by **DR-059**, 2026-08-24: eywa-deezy found 22 rows it could not categorise at all, and identified the cause correctly as a missing word rather than missing data — left alone, each brand would have filed its price pages under a different nearest category and every rule keyed on `page_category` would have gone quiet in a different way. 52 rows across the three brands now carry it. A template may legitimately hold more than one category: vth-biodent's T5 declares `[service_page, pricing_page]` and T6 declares `[knowledge_article, pricing_page]`, and that is not drift.)* Mostly determined by `content_format` but not a function of it. Backfilled on **all three brands** as of 2026-08-24: vth-biodent 686/761, deezy-dental 776/869, smile-scape-clinic 707/728 (189 NULL overall).
...
> 🩺 **`contraindications` เป็น store เดียวของข้อห้ามที่หน้าเว็บแสดง (DR-069, 2026-09-18)** — text[] ภาษาคนไข้ ≤ 8 ข้อ หนึ่งภาวะต่อข้อ เรียง strength 3→2→1 ไม่มีชื่อการค้า/ขนาดยา/link markup · แบรนด์ generate `src/data/entity-clinical.json` จากคอลัมน์นี้ (+ `seo_entity_devices.contraindications` + `seo_entity_drug.contraindications_text` — ชื่อต่างกัน bridge ซ่อนให้) แล้ว render + JSON-LD จากไฟล์เดียว · `load_source` ต่อท้าย PMID/ที่มาทุกครั้งที่แก้ · ไม่มี `indication` คู่กัน (ยังอยู่ใน YAML ของหน้า) · vth ตรวจแล้ว 40 entity / 568 ข้อ (2026-09-17/18) · แถวเป็นของแชร์ — แก้แถวที่แบรนด์อื่นโหลดต้องเก็บข้อเท็จจริงเดิม + broadcast (DR-046)
