...
> **Companion to:** คัมภีร์ EYWA™ PROTOCOL v3.12 (Part 5 + Part 2.6/2.7 + Part 10.7 Federation + Part 18.8 Two-Phase Sync + Part 18.9 Two-Column Identity + Part 25.11 Elementor + Part 27 Scoring + Part 28 Multilingual)  
> **Version:** v1.8 — 2026-05-08  
...
Major schema refinement based on production audit + operator feedback. Locks Phase 1 Supabase Database Foundation work.

**Headline Changes:**

- ➕ **Two-Column Identity Pattern** (DR-008):
  - `fingerprint text` — IMMUTABLE machine ID, format `{tablecode}_{ULID16}` (e.g., `ent_01HZP5K2XQR7N3MF`)
  - `fingerprint_display_name text` — MUTABLE human label, format `{fp_last_6}::{type}::{slug}::{key_data}`
  - Applied to all tables EXCEPT `seo_x_ads_keywords_contextual_master` (keeps existing format)
  - Auto-generated by `generate_ulid()` PostgreSQL function (pure SQL, no extensions)

- ➕ **Two-Tier Multilingual Strategy** (DR-009):
  - **Concept tables** (1 row + jsonb translations): entity, cluster, brand, author, doctor, branch, citation
  - **Content tables** (1 row per language + `translation_group_id`): page, keyword, review
  - jsonb pattern: `canonical_names`, `aliases`, `descriptions`, `brand_display_names`
  - `translation_group_id` format: `tg_{ULID16}`
  - `is_source_page boolean` flag — exactly 1 per translation group

- ➕ **Brand Scope Architecture** (DR-010):
  - `brand_scope text[]` for shared tables (entity, cluster, author, citation): `['*']`, `['vth-biodent']`, multi-brand arrays
  - `brand_slug text` (single, FK to brands) for brand-bound tables (page, doctor, branch)
  - All references use `brand_slug` (kebab-case, immutable) — replaces inconsistent UUID/notion_id usage

- ➕ **In-Place GTGT Upgrade Strategy** (DR-007):
  - Schema-only changes — data migration out of scope
  - Existing 30 migrations preserved as historical baseline
  - Idempotent additions (ADD COLUMN IF NOT EXISTS, CREATE TABLE IF NOT EXISTS)
  - Coexistence period: legacy + new patterns work side-by-side during transition

**Table Codes Standard (3-4 letters):**
- `ent` (entity), `page` (page), `clus` (cluster), `kw` (keyword — existing), `brnd` (brand)
- `auth` (author), `doc` (doctor), `brch` (branch), `cite` (citation)
- `pcit` (page-citation), `rev` (review), `aici` (AI citation tracking)
- `asc/chs/eas/eeat` (scoring), `gov` (governance audit), `kpi` (KPI baseline)
- `tg` (translation_group namespace, not a table)

**Helper Functions Added:**
- `generate_ulid()` — Pure SQL ULID generator (Crockford Base32, time-encoded)
- `generate_fingerprint_v2(p_tablecode text)` — Universal fingerprint creator
- `generate_entity_display_name()`, `generate_page_display_name()`, etc. — Per-table display formula
- `trg_set_fingerprint_*()` — Auto-generation triggers
- `trg_prevent_fingerprint_change()` — Immutability enforcement
- `trg_refresh_display_name_*()` — Display refresh on source data change

**Sections Updated:**
- All 9 group sections — table specs reflect two-column identity
- Appendix B (Fingerprint Patterns) — completely rewritten
- New Appendix E — Multilingual Strategy reference
- New Appendix F — Helper Functions reference

**Migration Impact:**
- Phase 1A: Add columns (5 migrations)
- Phase 1B: Create new tables (~14 migrations)
...
- `brnd_01HZP5K5AT0M6RHS` — brand
- `tg_01HZP5K6BU1N7SHT` — translation group
...
**Per-Table Composition:**

| Table | Formula | Example |
|-------|---------|---------|
| `ent` (entity_graph) | `{fp[-6:]}::{entity_type}::{entity_slug}::{icd_10_code}` | `N3MF::condition::sleep-apnea::G47.3` |
| `page` (page_master) | `{fp[-6:]}::{page_type}::{slug}::{page_language}::{brand_slug}` | `MFQR::pillar::airway-optimization::th::vth-biodent` |
| `clus` (cluster_master) | `{fp[-6:]}::{clinical_axis}::{cluster_slug}` | `QGR5::airway::sleep-disordered-breathing` |
| `brnd` (brands) | `{fp[-6:]}::{brand_slug}::{brand_name}` | `RHS6::vth-biodent::VTH BioDent` |
| `auth` (authors) | `{fp[-6:]}::{author_slug}::{full_name}` | `SHT7::dr-noppadol::Dr. Noppadol Rangsiyakul` |
| `doc` (brand_doctors) | `{fp[-6:]}::{brand_slug}::{doctor_slug}` | `TIU8::vth-biodent::dr-noppadol` |
| `brch` (brand_branches) | `{fp[-6:]}::{brand_slug}::{branch_slug}` | `UJV9::vth-biodent::sukhumvit-flagship` |
| `cite` (citations) | `{fp[-6:]}::{citation_type}::{citation_id}` | `VKW0::pmid::12345678` |

### Exception: Keyword Table

`seo_x_ads_keywords_contextual_master` keeps existing format:
...
**Why exception:**
- Keyword text is immutable (search terms don't change)
- 12,526 rows in production with active n8n flows
- Self-documenting (keyword visible in fingerprint)
- No display column needed — fingerprint serves both purposes

### Table Codes Reference
...
table_codes:
  ...
  kw:   seo_x_ads_keywords_contextual_master  # exception (existing format)
  rev:  seo_editorial_reviews
  
  # Junction / Time-Series Tables
  ...
  # Namespace Only (not a table)
  tg:   translation_group_id  # used in *_translation_group_id columns
```

### Cross-Check Pattern (Debug Aid)

The last 6 chars of `fingerprint` are embedded as the first segment of `fingerprint_display_name`. This creates a **double cross-check**:

```sql
-- Validation query: detect data integrity issues
...
-- ↑ Should return 0 rows. If not, trigger refresh failed.
```

**Real-World Debug Scenarios:**

1. **Data in wrong row**: workflow puts "tmj รักษา" data into "ฟันคุด" row
   - `fingerprint` says `kw_K7M3X9PQ`
   - `fingerprint_display_name` says `vitalsleep::th::th::ฟันคุด`
   - But row data says `tmj รักษา` → 🚨 mismatch detected

2. **Orphan reference**: page references entity that was renamed
   - `parent_fp = 'ent_01HZP5K2A'`
   - JOIN to entity reveals `display_name = 'condition::sleep-apnea::G47.3'` 
   - But page is about TMJ → 🚨 wrong parent

### Helper Functions Reference

```sql
-- Generate ULID (16 chars Crockford Base32)
SELECT generate_ulid();
-- → '01HZP5K2XQR7N3MF'

-- Generate fingerprint with table code
SELECT generate_fingerprint_v2('ent');
-- → 'ent_01HZP5K2XQR7N3MF'

-- Generate display name (entity)
SELECT generate_entity_display_name(
  'ent_01HZP5K2XQR7N3MF',
  'condition',
  'sleep-apnea',
  'G47.3'
...
**Old pattern (slug-based, mutable):**
```
fingerprint = "entity:sleep-apnea"
```
- ❌ Breaks when slug renamed
- ❌ Breaks when entity merged
- ❌ Cross-table FKs invalidated on rename

**New pattern (two-column):**
```
fingerprint              = "ent_01HZP5K2XQR7N3MF"  (immutable)
fingerprint_display_name = "n3mf::condition::sleep-apnea::g47.3"  (auto-refresh)
```
- ✅ Slug renames don't break references
- ✅ Entity merges/redirects clean
- ✅ Two-Phase Hierarchy Sync (DR-006) more robust
- ✅ Debug detection via cross-check
- ✅ ULID time-sortable for INSERT performance
- ✅ Compact (19 chars) vs UUID (36 chars)
- ✅ Pure SQL (no extensions)

### Migration from v1.7 to v1.8

For existing tables, schema migration adds 2 new columns initially nullable:

```sql
-- Phase 1A migration pattern
...
SET fingerprint = generate_fingerprint_v2('ent'),
    fingerprint_display_name = generate_entity_display_name(
      generate_fingerprint_v2('ent'),  -- new fp
      entity_type,
      entity_slug,
      icd_10_code
    )
...
## Appendix E — Multilingual Strategy (v1.8)

> **Added v1.8 (2026-05-08)** — Two-Tier Multilingual Pattern (DR-009). See also: Bible Part 28, DR-003.
...
**For brand-specific marketing variations:**

```sql
brand_display_names jsonb DEFAULT '{}'::jsonb
  -- Brand-specific marketing names per language (entity_graph only)
  -- Schema: {
...
source_translation_fp text  -- points to canonical row
translation_status text DEFAULT 'draft'  
  -- 'draft', 'in_progress', 'reviewed', 'published'
translation_due_date timestamptz
hreflang_validated boolean DEFAULT false
```

**Constraint: One source per group**

```sql
CREATE UNIQUE INDEX idx_one_source_per_tg
ON seo_website_page_master(translation_group_id)
WHERE is_source_page = true AND translation_group_id IS NOT NULL;
```

**Lifecycle:**

```yaml
step_1_create_first_translation:
  user_action: "Create page in Thai (canonical)"
  result:
    fingerprint: "page_01HZP5K2A"
    translation_group_id: "tg_01HZP5K2X"  # NEW group
    is_source_page: true

step_2_add_english_translation:
  user_action: "Translate to English"
  result:
    fingerprint: "page_01HZP5K2B"  # NEW unique
    translation_group_id: "tg_01HZP5K2X"  # SAME group
    is_source_page: false
    source_translation_fp: "page_01HZP5K2A"

step_3_add_japanese_translation:
  result:
    fingerprint: "page_01HZP5K2C"
    translation_group_id: "tg_01HZP5K2X"  # SAME
    is_source_page: false
    source_translation_fp: "page_01HZP5K2A"
```
...
```sql
-- Get all language versions of a page
SELECT 
  ...
  array_agg(json_build_object(
    ...
    'name', page_name,
    'is_source', is_source_page
  ) ORDER BY 
    CASE WHEN is_source_page THEN 0 ELSE 1 END,
    page_language
  ) as translations
...
SELECT 
  ...
  array_agg(distinct page_language) as languages
FROM seo_website_page_master p
LEFT JOIN logs_2026 l ON l.page_fp = p.fingerprint
GROUP BY translation_group_id;
```
...
|----------|--------|------|
| Does each language need unique URL? | Yes | Tier 2 |
| Does each language need unique SEO metadata? | Yes | Tier 2 |
| Is the data shared across languages (Wikidata, ICD-10, etc.)? | Yes | Tier 1 |
| Is the entity itself language-agnostic? | Yes | Tier 1 |
| Does each language have separate review/approval workflow? | Yes | Tier 2 |
| Are aliases per language meaningful? | Either | Both work |

### Migration from v1.7 to v1.8

For Tier 1 tables (entity, cluster, brand, etc.):

```sql
ALTER TABLE seo_entity_graph
  ADD COLUMN IF NOT EXISTS canonical_names jsonb DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS aliases jsonb DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS descriptions jsonb DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS brand_display_names jsonb DEFAULT '{}'::jsonb;

-- Backfill from existing single-value column
UPDATE seo_entity_graph
SET canonical_names = jsonb_build_object('en', entity_name)
WHERE canonical_names = '{}'::jsonb AND entity_name IS NOT NULL;

-- Migrate existing aliases text to jsonb
UPDATE seo_entity_graph
SET aliases = jsonb_build_object('en', string_to_array(aliases_text_old, ', '))
WHERE aliases_text_old IS NOT NULL;
```

For Tier 2 tables (page, etc.):

```sql
ALTER TABLE seo_website_page_master
  ADD COLUMN IF NOT EXISTS translation_group_id text,
  ADD COLUMN IF NOT EXISTS is_source_page boolean DEFAULT false;

-- For existing rows: each row gets its own group_id (assumes no translations exist yet)
UPDATE seo_website_page_master
SET translation_group_id = 'tg_' || generate_ulid(),
    is_source_page = true
...
## Appendix F — Helper Functions Reference (v1.8)

> **Added v1.8 (2026-05-08)** — Centralized helper function specifications.

### `generate_ulid()`

**Purpose:** Generate 16-character ULID (Crockford Base32, time-encoded)

**Signature:**
```sql
generate_ulid() RETURNS text
```
...
- Pure SQL (no extensions)
- Returns first 16 chars only (Pattern B compact)
...
BEGIN
  ...
  -- Encode 16 bytes to Base32 (26 chars), return first 16
...
**Purpose:** Universal fingerprint creator with table prefix

**Signature:**
```sql
generate_fingerprint_v2(p_tablecode text) RETURNS text
```

**Returns:** `{tablecode}_{ULID16}`

**Examples:**
- `generate_fingerprint_v2('ent')` → `'ent_01HZP5K2XQR7N3MF'`
- `generate_fingerprint_v2('page')` → `'page_01HZP5K3YR8M4PFQ'`
- `generate_fingerprint_v2('tg')` → `'tg_01HZP5K4ZS9L5QGR'` (translation group)

```sql
CREATE OR REPLACE FUNCTION generate_fingerprint_v2(p_tablecode text)
RETURNS text AS $$
BEGIN
  RETURN p_tablecode || '_' || generate_ulid();
END;
$$ LANGUAGE plpgsql VOLATILE;
```

### Per-Table Display Name Generators

**Purpose:** Generate `fingerprint_display_name` per table semantics
...
CREATE OR REPLACE FUNCTION generate_entity_display_name(
  ...
  SELECT lower(
    substring(p_fingerprint from length(p_fingerprint) - 5) || '::' ||
    coalesce(p_entity_type, '?') || '::' ||
    coalesce(p_entity_slug, '?') || '::' ||
...
```sql
CREATE OR REPLACE FUNCTION generate_page_display_name(
  p_fingerprint text,
  p_page_type text,
  p_slug text,
  p_page_language text,
  p_brand_slug text
) RETURNS text AS $$
  SELECT lower(
    substring(p_fingerprint from length(p_fingerprint) - 5) || '::' ||
    coalesce(p_page_type, '?') || '::' ||
    coalesce(p_slug, '?') || '::' ||
    coalesce(p_page_language, '?') || '::' ||
...
CREATE OR REPLACE FUNCTION generate_brand_display_name(
  ...
  SELECT lower(substring(p_fingerprint from length(p_fingerprint) - 5))
    || '::' || coalesce(p_brand_slug, '?')
...
```sql
CREATE OR REPLACE FUNCTION generate_cluster_display_name(
  p_fingerprint text,
  p_clinical_axis text,
  p_cluster_slug text
) RETURNS text AS $$
  SELECT lower(substring(p_fingerprint from length(p_fingerprint) - 5))
    || '::' || coalesce(p_clinical_axis, '?')
    || '::' || coalesce(p_cluster_slug, '?');
$$ LANGUAGE sql IMMUTABLE;
```

### Standard Triggers

**Auto-generate fingerprint on INSERT:**
...
**Auto-refresh display name when source changes:**
...
| Function | Returns | Mutability | Used For |
|----------|---------|------------|----------|
| `generate_ulid()` | text (16 chars) | VOLATILE | All fingerprints |
| `generate_fingerprint_v2(text)` | text | VOLATILE | Auto-generation triggers |
| `generate_entity_display_name(...)` | text | IMMUTABLE | Entity table display |
| `generate_page_display_name(...)` | text | IMMUTABLE | Page table display |
| `generate_brand_display_name(...)` | text | IMMUTABLE | Brand table display |
| `generate_cluster_display_name(...)` | text | IMMUTABLE | Cluster table display |
| `trg_set_fingerprint_*()` | TRIGGER | - | BEFORE INSERT |
| `trg_prevent_fingerprint_change()` | TRIGGER | - | BEFORE UPDATE (universal) |
| `trg_refresh_display_name_*()` | TRIGGER | - | BEFORE UPDATE OF source columns |
