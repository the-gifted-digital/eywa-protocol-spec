...
3. **ข้อความต่างกันตามจอได้ ภายใต้ h1 ตัวเดียว** — สามระดับ เรียงจากที่ควรใช้ก่อน:
   ...
   - **carousel / slider:** headline บนสไลด์**ไม่ใช่ h1** — ทำเป็น `<p>` ตกแต่ง แล้ววาง h1 ใน section แรกใต้สไลด์
     เป็น heading ตัวแรกใน `<main>` (vth หน้าแรก `b771759`) · ปุ่มคนละสไลด์ = คนละ id
...
หน้าแรก HeroSlider          2 → 1 (`b771759` — สไลด์เป็น <p> ตกแต่ง, h1 ย้ายลง section ใต้สไลด์, id แยกสามปุ่ม, exemption ถอดหมด)
...
**เพิ่มเติม 2026-08-24 (จาก eywa-deezy) — สัญญาณที่ 4 และ 5:**

5. 🔴 **เทียบ *ปี* ที่ label ระบุ กับปีจริงของเปเปอร์ (คลาด ±1 ได้)** — deezy พบว่า
   `smile-aesthetics-guide` ติด label ว่า *"de Geus JL … At-home vs in-office bleaching …
   Oper Dent. 2016."* บน PMID ที่จริงเป็น **Centenaro GG 2026** ซึ่งเป็น RCT เรื่องความเข้มข้น
   ไฮโดรเจนเปอร์ออกไซด์ — **ผิดคน ผิดวารสาร ผิดชนิดงาน ผิดปี 10 ปี** แต่**เกตปล่อยผ่าน**
   เพราะทั้งคู่เรื่องฟอกสีฟัน token ซ้ำกันพอให้ agreement = **0.50** เหนือ floor 0.34
   จึงไม่เคยเดินไปถึงชั้นเทียบชื่อผู้เขียนเลย
   **ปีเป็นจำนวนเต็ม** ไม่มีอะไรให้จูน ไม่ต้องเรียก API เพิ่ม และจับเคสนี้ได้ด้วยตัวเอง ·
   ตรวจ**ก่อน**เทียบชื่อเรื่อง เพราะประเด็นทั้งหมดคือชื่อเรื่องที่คะแนนสูงคือที่ซ่อนของแบบนี้ ·
   ±1 เผื่อ epub-ahead-of-print

6. **เทียบกับพี่น้องที่ locator เดียวกัน** — PMID 32383274 ถูกอ้างใน 26 หน้าว่า Sanz (EFP S3 2020)
   และอีก 1 หน้าว่า Chapple (consensus 2018) · 26 ต่อ 1 ตัดสินได้ว่าฝั่งไหนผิดโดย**ไม่ต้องถามใคร**
   · **ถูกที่สุดในสามสัญญาณ ไม่ต้องต่อเน็ตเลย** จึงยังทำงานตอน PubMed/Crossref ล่ม ซึ่งเป็นสถานการณ์
   ที่อีกสองสัญญาณปฏิเสธจะสรุปอะไรอย่างถูกต้อง · **เตือนอย่างเดียว ไม่บล็อก** — เสียงข้างมากคือหลักฐาน
   ไม่ใช่ข้อพิสูจน์ และต้องมีข้างมากอย่างน้อย 3 เท่า (2 ต่อ 2 ไม่พูดอะไร)
...
**Scope:** **UNIVERSAL** — เป็น *วิธี* ไม่ใช่ config ของ Deezy · เกี่ยวกับ DR-054 ตรงที่หลายข้อแตะชั้น CTA จึงรีเซ็ตนับ stability ของระบบวัด

**Context:**

หน้าแรก Deezy ได้ **82** บน Lighthouse mobile แต่ ณ เวลาเดียวกัน หน้าเดียวกัน CrUX field data ของคนจริงคือ **LCP p75 1,577 ms · CLS 0.01** — ผ่าน "Good" ทั้งคู่สบาย ๆ

Google จัดอันดับด้วย field data ส่วนคะแนน lab คือ Moto G Power จำลองบน Slow 4G + หน่วง CPU 4× งานทั้งหมดข้างล่างนี้จึงไม่ด่วน และกฎข้อแรกออกมาจากตรงนั้น

> **อ่าน field data ก่อนแตะอะไร ถ้า CrUX ผ่านอยู่แล้ว การแก้ที่ต้องจ่ายด้วยผู้ใช้จริงหรือข้อมูลจริง ไม่คุ้มกับตัวเลข lab**

ส่วนใหญ่ของที่ทำก็ช่วยคนจริงด้วย จึงคุ้มอยู่ดี — แต่การแก้ที่ **แลกคุณภาพการวัดกับแต้ม** เป็นการแลกที่แย่ และเป็นการแลกที่พลาดง่ายที่สุดโดยไม่ตั้งใจ

**Decision:**

1. **อ่านคะแนนแยกรายเมตริก ไม่ใช่ตัวเลขรวม** — 82→98 เกือบทั้งหมดคือ LCP ส่วนแต้มสุดท้ายที่เหลือเป็น FCP ล้วน ๆ สองปัญหาใส่ตัวเลขเดียวกัน แต่ละอันอยากได้คนละยา
2. **ซ่อม paint path ให้จบก่อนแตะ third-party** — 82 → 92 → 98 มาจากของตัวเองล้วน
3. **defer tag manager ทีหลังสุด และตรวจแยกต่างหาก** — ราว 6 แต้ม
4. **แล้วค่อยถามว่าถอดแท็กทิ้งเลยได้อะไรไหม** — ที่ 98 คำตอบของ Deezy คือ **~0 แต้ม** เพราะแท็กไม่แตะเมตริกที่ให้คะแนนแล้ว การเช็คข้อเดียวนี้ประหยัด migration 1–2 สัปดาห์ที่แบกความเสี่ยงกับข้อมูลจริง — **รันก่อนเริ่มโปรเจกต์นั้น ไม่ใช่หลัง**

**Rationale — สิ่งที่ขยับตัวเลขจริง:**

- **preload รูป LCP** — `fetchpriority="high"` บน `<img>` แค่จัดลำดับ *หลัง* เบราว์เซอร์เจอ element ทำให้เจอเร็วขึ้นไม่ได้ ค่านั้นคือ resource load delay 464 ms สำหรับรูปที่โหลดจริง 0.4 ms · ใช้ `<link rel="preload" as="image">` **หนึ่งอันต่อหนึ่ง breakpoint ของ art direction พร้อม `media` ของตัวเอง** — preload อันเดียวโหลดทุก candidate แล้วจ่ายที่ประหยัดได้คืนสองเท่า
- **defer GTM ไปที่ `load` หรือ interaction แรก แล้วแต่อะไรมาก่อน** — 279 KiB + ~250 ms main thread อยู่ในหน้าต่างก่อน LCP · `load` = 5–6 s ใน lab แต่ ~1.6 s ในสนาม แท็กจึงออกนอกหน้าต่างที่วัดโดยแทบไม่มีต้นทุนกับคนจริง · trigger ที่ interaction คือสิ่งที่รักษา conversion ของคนกดเร็วไว้ใน GA4 และ Lighthouse ไม่เคยกดอะไร จึงไม่มีต้นทุนใน lab
- **เลิกเขียน class ทุก scroll event** — `classList.toggle(name, bool)` เขียน attribute ไม่ว่าค่าจะเปลี่ยนหรือไม่ · class นั้นเปลี่ยนความสูงของ element ทุก scroll จึงล้ม layout ~2,000 element = forced reflow 152 ms จากบรรทัดเดียว
- **trigger งานใต้ fold ด้วย scroll ไม่ใช่ timer** (ดูกับดัก)
- **animation ที่ composite อย่างเดียว** — keyframe `box-shadow` / `background-position` repaint บน main thread ตลอดกาล ใช้ `transform`/`opacity` บน pseudo-element ได้ผลเดียวกันโดยไม่ repaint
- **ขนาด DOM** — review card 128 → 80 และใช้ `<symbol>` + `<use>` แทน SVG 4 path ต่อการ์ด (512 path element สำหรับ glyph 12 px อันเดียว) · 2,833 → 1,969 element

**Consequences — กับดัก แต่ละข้อจ่ายไปแล้วหนึ่งรอบวัด:**

- ⚠️ **เลื่อน paint ใต้ fold ไปใส่ timer = ย้ายต้นทุน ไม่ใช่เอาออก** — ย้าย fetch จาก `DOMContentLoaded` ไป `load` + `requestIdleCallback` แก้ LCP ได้แต่ดัน **Speed Index 2.6 s → 4.1 s** เพราะมันยังรันอยู่ใน trace และการวาดช้าคือสิ่งที่ SI วัดพอดี · trigger ที่ scroll แรกเอาออกจาก lab run ได้หมดเพราะ Lighthouse ไม่ scroll — และเป็นกฎที่ซื่อสัตย์ด้วย คนที่ไม่ scroll ก็ไม่มีวันเห็นมันอยู่แล้ว
- ⚠️ **วัด delta หลัง gzip ก่อนลงมือทำงานระดับไบต์** — dedupe SVG ซ้ำทั้งหมดดูเหมือน 15.5 KB หลัง gzip เหลือ **2.1 KB** ≈ 11 ms บน Slow 4G · gzip ยุบความซ้ำให้อยู่แล้ว (sprite อันที่คุ้มจริงตัด 512 **DOM element** ซึ่ง gzip ช่วยไม่ได้ — นั่นคือเส้นแบ่ง)
- ⚠️ **แยก CSS ออกจาก HTML อาจไม่ประหยัดอะไรเลย** — วัดทั้งสองทาง: inline 66.2 KB gz เทียบ external 41.4 + 25.4 = 66.8 KB gz ไบต์เท่ากัน แถมได้ request ที่บล็อก render มาอีก 7 อัน · ทดสอบกับไซต์จริง อย่าเดา
- ⚠️ **Response ของ Worker เองไม่ถูก edge-cache โดย Cloudflare** — ใส่ `Cache-Control` เฉย ๆ ไม่มีผล ต้องอ่าน/เขียน `caches.default` เอง ไม่งั้นผู้เข้าชมทุกคนจ่ายค่า list bucket สด
- ⚠️ **`display: none` ไม่หยุดรูป eager** — wrapper `md:hidden` ยังโหลด hero มือถือทุกครั้งที่เปิดบน desktop · ใช้ `<picture>` + `media` บน `<source>` และ transparent pixel เป็น `<img src>`
- ⚠️ **redirect ที่เพิ่ง deploy อาจดูเหมือนยังไม่ deploy** — edge cache ถือ response ก่อน deploy โดย key ตาม scheme ได้ · ตรวจงานชั้น edge ด้วย cache-buster และยิงมากกว่าหนึ่ง path
- ⚠️ **อ่าน output ของเครื่องมือให้ครบ** — เคยรายงานว่า `astro check` ผ่าน เพราะรันผ่าน `tail -3` ซึ่งตัดบรรทัด `- 1 error` ทิ้งเหลือแต่จำนวน warning · error นั้นอยู่บน branch มาหลายวัน และทุก deploy ตั้งแต่นั้น ship จาก tree ที่ type-checker ปฏิเสธ เพราะไม่มีอะไรใน CI รันมัน
- ❌ **สิ่งที่ตั้งใจไม่ทำ:** Partytown (GA4 ใน worker ตั้ง first-party cookie เองไม่ได้ `client_id` เสื่อม แล้ว join `bond_id` ↔ `ga_client_id` ที่ DR-054 พึ่งอยู่พัง) · ทิ้ง tag manager ไปใช้ gtag เปล่า (ประหยัดไบต์ แต่ทำให้ทุกการแก้แท็กกลายเป็น code deploy ซึ่งตามกฎของ DR-054 เองรีเซ็ตนับ stability ทุกครั้ง) · critical CSS (คันโยกเดียวที่เหลือสำหรับแต้มสุดท้าย แต่เสี่ยง FOUC บนไซต์คลินิกที่ field data ผ่านอยู่แล้ว)
- 📌 **ถอด JS ของ Google ทิ้งทั้งหมด** (collector first-party + Measurement Protocol จาก Worker) ยังเป็นทางเดียวที่ผ่าน ~98 ในกรณีทั่วไป และมีประโยชน์ที่ **ไม่ใช่คะแนน** จริง ๆ (ad blocker ทิ้ง client-side hit 10–30% แต่แตะ server-side ไม่ได้) · ทำในฐานะ **โปรเจกต์ความครบของข้อมูล** โดย dual-send เข้า shadow property จนกว่าจะ reconcile กับ `tsa_bond` ได้ · ห้ามทำในฐานะโปรเจกต์ performance และห้ามสลับตรง ๆ
- ✅ **วิธีเล่าผลอย่างซื่อสัตย์:** เรื่องที่ฟังดูสมเหตุสมผล — *"เราแลกแต้มกับ tracking ที่ละเอียด"* — **ผิด และตรวจสอบได้** · ที่ 98 คะแนนแตกเป็น FCP 9/10 · LCP 25/25 · TBT 30/30 · CLS 25/25 · SI 10/10 ที่หักทั้งหมดคือ FCP ซึ่งเป็น CSS ของเราเอง · tag manager ได้คะแนนสะอาด และบรรทัด "Reduce unused JavaScript" ของ Google เองสำหรับมันถูกทำเครื่องหมาย **Unscored** · ข้อที่ควรพูดจึงไม่ใช่ "ยอมจ่ายต้นทุนไป" แต่คือ **analytics เต็มระบบไม่ต้องมีต้นทุนก็ได้ และนี่คือการวัดที่พิสูจน์**

**References:** DR-054 (Tsaheylu) · DR-055 (CTA attribution) · working notes เต็ม `eywa-deezy/docs/web-performance.md` · staging draft `eywa-deezy/docs/proposed-protocol-changes/DR-056-web-performance-method.md`

---

### [DR-055] — CTA attribution: คำศัพท์ปิดสำหรับ "ที่ไหน" · ข้อตกลงเปิดสำหรับ "ปุ่มไหน" · และคอมโพเนนต์ที่ลืมไม่ได้ (2026-08-12) 🔒🎯🧩

**Status:** **🔒 Locked 2026-08-14** — operator-directed · ทุกกฎในนี้เขียนวันที่มันพังใน production · รวมบันทึกย้อนหลังของการเปลี่ยนคำศัพท์ระดับ federation ที่ลงไปแล้ว (ดู *ที่ลงไปก่อนมีบันทึกนี้*)

**Scope:** **UNIVERSAL** — ผูกพันทุกแบรนด์ตั้งแต่วินาทีที่มันเรนเดอร์ CTA แปลงผล · ควบคุม `tsa_param.allowed_values` ของ `cta_location` (ต่อจาก DR-054)

**Context:**

ลิงก์ conversion ที่ ship โดยไม่มี attribution **ยังแปลงผลได้ปกติ** — listener ยิง bond ถูกเขียน GA4 ได้ event ไม่มีอะไรดูเสีย ที่หายไปคือ **ชื่อ** เท่านั้น: คลิกไปลงที่ `cta_location: 'unmarked'` กับ `cta_id` ว่าง แล้วคลิกไร้ชื่อทั้งไซต์ยุบรวมเป็นถังเดียวที่แยกกลับไม่ได้อีกเลย

สองเหตุการณ์วันเดียวกัน คนละทิศ:

- **34 หน้า hyper-local §9** ship โดยปุ่ม LINE ซึ่งเป็นปุ่ม *หลัก* ไม่มีแท็ก ส่วนปุ่มโทรข้าง ๆ มี `data-cta` มาตั้งแต่แรก — มีคนแท็กไปหนึ่งในสอง · มี conversion ลงไปแบบไม่ระบุตัวตนแล้ว 3 ครั้ง
- **popup ตามกำหนดการ** ถูกสร้างพาด 65 หน้าด้วยความพลาดแบบเดียวกัน จับได้ก่อน deploy

ทั้งคู่ไม่ใช่การตั้งชื่อผิด ทั้งคู่คือ **attribute ที่ลืม** ความต่างข้อนี้ตัดสินการออกแบบทั้งหมด

**Decision:**

1. **`cta_location` = ปิด · governed · ทั้ง federation** — surface `ga4_dim` · cardinality **ต่ำโดยตั้งใจ** (31 ค่า) · อำนาจอยู่ที่ `tsa_param.allowed_values` ซึ่ง **ไม่มี `brand_id`** = คำศัพท์ชุดเดียวสำหรับทุกแบรนด์ · การเพิ่มค่าเป็น **การตัดสินใจระดับ federation ไม่ใช่ผลพลอยได้ของการเขียนคอมโพเนนต์** ถ้าแต่ละแบรนด์คิดตำแหน่งของตัวเอง ก็เทียบข้ามแบรนด์ไม่ได้ ซึ่งเป็นเหตุผลที่ตารางร่วมมีอยู่
2. **`unmarked` มีอยู่ในข้อมูล แต่ห้ามเขียนเอง** — มันคือสิ่งที่ client เขียนตอนไม่มี `data-cta` เลย · อนุญาตให้ใส่ใน markup ได้เมื่อไหร่ ก็กลายเป็นช่องขอไม่ถูกวัดที่ผ่านเกตได้ · vocabulary ที่ generate ออกมากรองมันทิ้ง
3. **`cta_id` = เปิด · มีข้อตกลง · ต่อแบรนด์** — surface **`bq_only`** ไม่ส่งเป็น GA4 dimension เลย · cardinality สูงและคาดว่าจะโต · ข้อตกลง `<component>-<what>` (`popup-line`, `servicehero-primary`, `branchhero-tel`) · **ห้ามใช้ id ซ้ำข้ามตำแหน่ง** สแกนจะ fail
4. **แคตตาล็อกปิดของชื่อปุ่มถูกพิจารณาแล้วและปฏิเสธ** — มันไม่กันความพลาดที่เกิดจริง (แคตตาล็อกของชื่อไม่ช่วยอะไรกับ attribute ที่ลืม) และสร้างแรงกดดันที่แย่กว่า: ที่ใหม่โผล่ตลอด แคตตาล็อกเก่า ทางที่ง่ายที่สุดกลายเป็น **หยิบ id เดิมมาใช้ซ้ำ** ซึ่งรวมปุ่มสองปุ่มเป็นแถวเดียวในทุกรายงาน อย่างถาวร · **id ใหม่ปลอดภัยกว่า id ที่ยืมมาเสมอ** และเพราะ `cta_id` เป็น `bq_only` จึงไม่มีเพดาน cardinality ของ GA4 ให้ชน — นี่คือเหตุผลที่มันเปิดได้
5. **บังคับสามชั้น เร็วสุดก่อน** — (1) คอมโพเนนต์ `CtaLink` จับ prop ที่ขาดและ `ctaLocation` นอกคำศัพท์ **ตั้งแต่ในเอดิเตอร์** เพราะเรียกโดยไม่ใส่ prop ไม่ได้ และ type ผูกกับ union ที่ generate มา · (2) `astro check` จับซ้ำใน CI · (3) `scan:tracking` แบบบล็อก จับทุกอย่างที่เรนเดอร์นอกคอมโพเนนต์ (template string ใน JS, markup เก่า, การพอร์ตจากอีกแบรนด์) หลัง build ก่อน deploy · เก็บทั้งสาม: สแกนเป็นตัวเดียวที่เห็น HTML ที่ build แล้ว ซึ่งคือสิ่งที่ผู้เข้าชมได้จริง
6. **`CtaLink` ตัดสิน `target`/`rel` จาก href ด้วย** — `tel:` ต้องไม่เปิดแท็บใหม่ ลิงก์ LINE ภายนอกต้องเปิด · พลาดตรงนี้เสีย conversion บนมือถือแบบเงียบ ๆ จึงตัดสินครั้งเดียวที่ศูนย์กลาง

**Rationale — แหล่งความจริงเดียวของคำศัพท์:**

สแกนเนอร์ตอน build ต้องใช้ลิสต์เดียวกับที่ตาราง governance ถือ เดิมมันเก็บ **สำเนาที่พิมพ์มือ** พร้อมคอมเมนต์สัญญาว่าจะทำ sync ซึ่งไม่เคยมาถึง · ถึง 2026-08-12 governance มี 30 ค่า สำเนามี 23: `care_hero` · `doctor_hero` · `lp_header` · `lp_price` · `lp_trust` · `lp_closing` เป็นคำศัพท์ที่อนุมัติแล้วแต่สแกนเนอร์ปฏิเสธ

ไม่มีพิษภัยตอนสแกนเนอร์แค่เตือน **แต่วินาทีที่มันกลายเป็นเกตแบบบล็อก มันกลายเป็นกับดัก** — CTA ที่แท็กถูกทำให้ deploy ล้ม และทางออกที่เร็วที่สุดคือเปลี่ยนป้ายเป็นค่าที่สแกนเนอร์บังเอิญรับ ซึ่งทำลาย dimension เงียบ ๆ แทนที่จะแก้อะไร

> **กฎ: ลิสต์ใดที่ต้องตรงกับตาราง governance ให้ generate จากมัน ห้ามพิมพ์ซ้ำ**

`gen-cta-locations.mjs` → `src/lib/cta-locations.ts` (commit ไว้ เพื่อให้ build และ type-check ในเครื่องไม่ต้องพึ่ง DB) regenerate ใน CI ข้าง `page-context.json` ทุก deploy

**Consequences:**

- ✅ **หลักฐานว่าได้ผล:** popup ถูกสร้างใหม่บน `CtaLink` แล้วผู้เขียนแยก `popup-tel` ออกจาก `popup-line` เองโดยดูจาก href ที่ resolve — กฎ "ห้ามใช้ id ซ้ำข้ามช่องทาง" ถูกใช้โดยไม่ต้องอ้างถึง · ลิงก์ conversion ที่ไม่มีแท็กทั้งไซต์ 65 → **0**
- ⚠️ **ที่ลงไปก่อนมีบันทึกนี้:** `popup` ถูกเพิ่มเข้า `tsa_param.allowed_values` ของ `cta_location` เมื่อ 2026-08-12 (30 → 31 ค่า) เพื่อปลดล็อก popup ที่ ship วันเดียวกัน · เพราะ `tsa_param` ไม่มี `brand_id` **นั่นคือการเปลี่ยนคำศัพท์ของทั้ง 15 แบรนด์** · เป็นการเพิ่ม ไม่ทำลายอะไร และสอดคล้องกับกฎข้างบน — แต่เป็นการกระทำระดับ federation ที่ทำโดยไม่มีบันทึก และย่อหน้านี้คือบันทึกนั้น · **ลำดับต่อจากนี้: `tsa_param` ก่อน → regenerate → แล้วค่อยเขียนคอมโพเนนต์**
- ⚠️ **แบรนด์ที่รับไปใช้ต้องรัน `gen:cta-locations` หนึ่งครั้งและ commit ผลก่อนเปิดสแกนแบบบล็อก** ไม่งั้นมันจะปฏิเสธคำศัพท์ที่ governance ถือว่าถูก
- ⚠️ **แบรนด์ที่มี CTA ไม่มีแท็กอยู่เดิม ให้คาดว่าเกตจะ fail ตั้งแต่รันแรก** — นั่นคือจุดประสงค์ ไปแท็กมัน
- 📌 markup เดิมยังทำงานปกติ `data-cta` / `data-cta-id` ไม่เปลี่ยน · `CtaLink` บังคับกับลิงก์ conversion **ใหม่** ไม่ใช่คำสั่งให้เขียนใหม่ทั้งไซต์

**References:** DR-054 (Tsaheylu) · DR-046 (shared-table governance — `tsa_param` เข้าเงื่อนไขเดียวกัน) · reference implementation `eywa-deezy/web/src/components/ui/CtaLink.astro` + `web/scripts/gen-cta-locations.mjs` + `web/scripts/tracking-scan.mjs` · staging draft `eywa-deezy/docs/proposed-protocol-changes/DR-055-cta-attribution-governance.md`

---

### [DR-054] — EYWA Tsaheylu: การวัด conversion เป็นระบบของ federation ไม่ใช่การติดแท็กรายแบรนด์ (2026-08-12) 🔒🌐📊

**Status:** **🔒 Locked 2026-08-14** — operator-directed · สร้างและรันจริงบน production ของ Deezy ตั้งแต่ 2026-07-27 · 300 bond ใน 16 วัน · ครบหนึ่งรอบ stability และจับ regression ได้หนึ่งตัวในสนาม · **ทุกกฎข้างล่างจ่ายค่ามาแล้วหนึ่งครั้ง**

**Scope:** **UNIVERSAL** — ทุกแบรนด์ที่มี CTA แปลงผล · ติดตั้งได้เลยวันนี้ ตาม checklist §18

**Schema Reference:** `tsa_bond` · `tsa_param` · `tsa_event` · `tsa_product_map` · `tsa_product_value` · view `tsa_bond_valued` · `brands.tsa_phase` · migration `38`–`44`

**Context:**

คำถามเชิงธุรกิจทั้งหมดของไซต์คลินิกคือ *คอนเทนต์ไหนสร้างคนไข้* การตอบต้องใช้ข้อเท็จจริงสามอย่างที่ join กัน ณ วินาทีที่เกิด conversion: **หน้านี้เป็นหน้าประเภทไหน** · **คนเข้ามาถึงตรงนี้ยังไง** · **เขากดปุ่มไหน** GA4 อย่างเดียวให้ไม่ได้สักอย่างอย่างเชื่อถือได้ — event `line_click` ที่ไม่มีตัวตนของหน้า คือตัวเลขที่ไม่มีตัวหาร

ที่แย่กว่านั้น tracking พังแบบ **เงียบ** — CTA ship โดยไม่มี attribution แล้วไซต์ยังทำงาน conversion ยังเกิด dashboard ยังมีตัวเลข ไม่มีใครรู้ว่ามีอะไรผิดไปทั้งไตรมาส และถึงตอนนั้นข้อมูลสร้างกลับไม่ได้แล้ว **ทุกเหตุการณ์ในบันทึกนี้เป็นรูปทรงนี้ทั้งหมด**

Deezy ยังสาธิตกรณี null ให้ดูด้วย: cutover 2026-07-20 ship โดยไม่ได้ตั้ง `PUBLIC_GTM_ID` ใน production ไซต์ **มืดสนิทด้านการวัด 8 วัน** ≈ 560 session ที่ไม่มีอยู่ที่ไหนเลย และจะไม่มีวันมี

**Decision:**

1. **คำศัพท์ตายตัว ต่อรองไม่ได้ ข้ามแบรนด์** — **Tsaheylu** (ระบบ) · **TSA** (prefix โค้ด: ตาราง `tsa_*`, `DR-EYWA-TSA-NNN`, `npm run tsa:*`) · **Signal** (event ที่พก context) · **Bond** (หนึ่งเส้นทางที่ตามรอยได้: เข้า → คลิก → (Phase 5) ยืนยัน add → คนไข้) · **bond_id** (คีย์ join สร้างฝั่ง client ไปลงทั้ง event ของ GA4 *และ* แถวฝั่ง server) · การดริฟต์ตรงนี้แพงมากตอนแก้ทีหลัง และถูกมากตอนกันไว้ตอนนี้
2. **สาม context layer บนทุก signal** — **A (หน้า)** `page_fingerprint` · `cluster_id` · `product_line` · `primary_entity` · `page_type` · `page_section` · `branch_slug` · `content_locale` จาก `page_master` ตอน build · **B (เส้นทาง)** `entry_page` · `entry_channel` · `entry_campaign` · `pages_before_convert` · `journey_path` + gclid/utm ที่ snapshot ตอนเริ่ม session จาก sessionStorage · **C (การกด)** `cta_location` · `cta_id` · `cta_label` · `scroll_at_click` · `seconds_to_click` · `bond_id` ตอนคลิก
3. **Layer A ต้องขี่ไปกับ `page_view` ด้วย ไม่ใช่เฉพาะ conversion** — ไม่งั้นมีตัวเศษที่ไม่มีตัวส่วน และคำนวณ conversion rate ต่อ product ไม่ได้ · **นี่คือขั้นที่ถูกข้ามบ่อยที่สุด**
4. **click router คือหัวใจ** — `/go/line` (302 + แถว) และ `/go/tel` (beacon + แถว) เขียน `tsa_bond` **ฝั่ง server** ไม่ใช่ของประดับ: ad blocker และ ITP ลบ client-side GA4 hit ทิ้ง 10–30% แต่แตะ navigation first-party บน origin ของเราเองไม่ได้ · ไม่มีเพดาน cardinality จึงเก็บค่าดิบได้ในที่ที่ GA4 รับได้แค่ bucket · `bond_id` ทำให้ Phase 5 (ยืนยันผ่าน LIFF) เป็น JOIN ไม่ใช่การติดตั้งใหม่
5. **redirect ต้องเกิดแม้การเขียนจะล้ม** — insert ล้มเสียหนึ่งแถว redirect ล้มเสียคนไข้หนึ่งคน · 302 คืนก่อน แล้ว insert รันใน `waitUntil`
6. **`tsa_bond` ⊇ GA4 — วัดแล้ว ไม่ใช่สมมติ** · reconcile บน `bond_id` 2026-07-29: GA4 23 · `tsa_bond` 24 · ตรงกัน 23 · **อยู่ใน GA4 อย่างเดียว 0** ส่วนเกินหนึ่งคือแถวที่อ่าน cookie `_ga` ไม่ได้ · ยืนยันซ้ำ 08-04 (12=12) · 08-05 (19=19) · 08-06 (15=15) → **ทุก conversion ที่ GA4 รู้ Supabase รู้ · ทางกลับไม่จริง** · กฎการอ่านที่ตามมา: จำนวน conversion สัมบูรณ์ → `tsa_bond` (GA4 นับขาดโดยโครงสร้าง) · อัตรา → คำนวณภายในระบบเดียวตั้งแต่ต้นจนจบ (เอาตัวเศษ `tsa_bond` หารด้วยตัวส่วน GA4 = ผสมตัวเศษเกือบครบกับตัวส่วนที่รั่ว ทำให้อัตราสูงเกินจริง) · export เข้าแพลตฟอร์มโฆษณา → GA4 (surface เดียวที่ทำได้ และการนับขาดของมันสม่ำเสมอ bidding จึง optimize ถูกอยู่) · **reconciliation ยืนยัน CONTAINMENT ไม่ใช่ความเท่ากัน** เตือนเฉพาะ `bond_id` ที่มีใน GA4 แต่ไม่มีใน Supabase เพราะทิศนั้นแปลว่าพังจริง
7. **ขอบเขต federation อยู่ใน schema** — `tsa_bond` มี `brand_id` (แบรนด์เขียนพร้อมกัน PK คือ `bond_id` สุ่ม) · `tsa_product_map` PK `(brand_id, cluster_id)` เพราะ **cluster ใช้ร่วมทั้ง federation แต่ความหมายเชิงพาณิชย์ของมันไม่ใช่ ห้ามเอา mapping รายแบรนด์ไปแปะบนแถว cluster ที่ใช้ร่วม** · `tsa_product_value` PK `(brand_id, product_line)` เพราะราคาเป็นของแต่ละแบรนด์ · **`tsa_param` และ `tsa_event` ไม่มี `brand_id` โดยเจตนา** — คำศัพท์เป็นของทั้ง federation ชื่อ parameter และ event ต้องเหมือนกันทุกที่ ไม่งั้นรายงานข้ามแบรนด์ไม่ได้เลย (ดู DR-055)
8. **value เป็น join ไม่ใช่ตัวเลขที่อบมากับแถว** — `tsa_product_value` + view `tsa_bond_valued` · พิสูจน์โดยตั้งรากเทียมเป็น 45,000 แล้วเห็น bond เก่าสามอันตีค่าใหม่เป็น 135,000 บาท แล้วย้อนกลับ · placeholder ใช้ได้เฉพาะเมื่อ `is_placeholder = true` — ธงนี้คือสิ่งที่กันไม่ให้ใครรายงานเรื่องแต่ง
9. **launch protocol ไม่ใช่ทางเลือก** — รัน `daily-check.sql` ทุกวันตั้งแต่ CTA แรกจนกว่าเครื่องมือจะนิ่ง อ่านใน Supabase ไม่ใช่ GA4 เพราะ router เขียนแบบ synchronous ส่วน BigQuery export ช้าไปหนึ่งวัน — ในสัปดาห์เปิดตัวนั่นคือความต่างระหว่างแก้บั๊กกับเจอมันหลังข้อมูลหายไปแล้วหนึ่งสัปดาห์ · **เงื่อนไขออก:** สามวันติดที่ไม่มี repeat tap · ไม่มีแถวทดสอบ · ไม่มี `cta_id`/`entry_channel` หาย · และปริมาณที่อธิบายได้ · **deploy ที่แตะชั้น CTA รีเซ็ตการนับ** เพราะนั่นคือจังหวะที่ความพังเงียบเข้ามาพอดี
10. **Layer A ต้องดูแลตัวเอง** — `page-context.json` คือ export ของ `page_master` ที่ commit ไว้ ขั้น deploy เดิม regenerate แล้ว **โยนผลทิ้ง** เพื่อให้ build reproduce จาก commit ได้ ผลคือตรงข้ามกับความปลอดภัย: Layer A เสื่อมทุกครั้งที่คอนเทนต์ ship และคอนเทนต์ ship ตลอดเวลา · 2026-08-12 พบ 34 หน้า §9 live และแปลงผลอยู่โดย **ไม่มีตัวตนของหน้าเลย** ไม่มี fingerprint ไม่มี cluster ไม่มี product line · conversion สามครั้งลงไปแบบไม่ระบุที่มาแล้ว และคำเตือน drift พิมพ์อยู่ใน CI มาหลายวัน · **กฎ: build ต้องใช้สิ่งที่มัน regenerate** หน้าใหม่พกตัวตนของมันมาตั้งแต่ deploy แรกที่เผยแพร่มัน โดยไม่มีขั้นตอนที่ต้องใช้คน เพราะแถวในแผนมีอยู่ใน `page_master` ก่อนที่หน้าจะมี · ไม่มีคีย์หรือ generator ล้ม ให้ fallback ไปไฟล์ที่ commit ไว้ · **และ coverage scan บล็อก deploy** หลัง refresh มันจะ fail ได้แค่สองอย่างที่ระบบอัตโนมัติแก้ไม่ได้: หน้า live ที่ไม่มีแถวใน `page_master` และลิงก์ conversion ที่ไม่มี `data-cta` ซึ่งทั้งคู่สร้าง conversion ที่ระบุที่มาไม่ได้อย่างถาวร (ดู DR-055)

**Rationale — สองนิยามที่แบรนด์ต้องรับก่อนรายงานอะไรก็ตาม:**

**จำนวน conversion ไม่ใช่จำนวนแถว** · `tsa_bond` บันทึกทุกคลิก บางอันคือคนเดียวกดสองปุ่มคนละปุ่มบนหน้าเดียวภายในหนึ่งนาที — 17 จาก 337 bond แรก ≈ 5% · guard ฝั่ง client **จงใจไม่กด** พวกนั้นทิ้ง: มันยุบเฉพาะปุ่มเดียวกันบนหน้าเดียวกัน คนที่กำลังเทียบสองการรักษาจึงเก็บ conversion ไว้ทั้งคู่ และลำดับ A→B รอดในฐานะข้อค้นพบด้าน UX ว่าปุ่มแรกคือปุ่มที่ปิดการขายไม่ลง · การยุบเป็นงานของ **รายงาน** ตามหลักการของระบบนี้เอง — บันทึกทุกอย่าง เลือกเกณฑ์ตอนอ่าน · คิวรีนับมาตรฐานอยู่ใน `docs/analytics/README.md` แบรนด์ที่นับแถวแทนจะรายงาน conversion เกินจริงประมาณ 5% นั้น

**`no_cookie` เป็นเทรนด์ ไม่ใช่เกต** · bond ที่อ่าน cookie `_ga` ไม่ได้ยังพกช่องทาง product line cluster และ CTA ครบ หายแค่การ join กลับ GA4 · **นับเป็น device** — ระดับแถวอ่านได้ 17% ระดับ device 8.1% เทียบกับ 6.8% ตอนโหลดแท็กทันที คือความต่างของ device เดียวที่แต่งตัวมาเป็น regression 4 เท่า · ที่วันละ 10–15 conversion เมตริกนี้แยก 7% จาก 12% ไม่ได้ไปอีกหลายเดือน และมันขึ้นเองตามการแพร่ของเบราว์เซอร์สายความเป็นส่วนตัว ซึ่งเป็นตัวแปรกวนที่ไม่มีใครคุม · escalate เฉพาะเมื่อเกิน 20% ของ device ต่อเนื่อง 30 วันบน ≥200 device · ถ้าถึงคราวต้องตัดสินจริง อย่าจ้องมันให้หนักขึ้น — รัน A/B 50/50 ของจังหวะโหลดแท็กใน Worker หนึ่งสัปดาห์ ซึ่งตัดตัวแปรกวนทิ้งหมดและตอบได้ใน 7 วัน · **การตั้งสายสะดุดบนตัวเลขที่ noise ขนาดนี้ คือวิธีที่ระบบนี้เคย revert การเปลี่ยนแปลงที่ทำงานได้อยู่แล้วมาแล้วหนึ่งครั้ง**

**Consequences — กับดัก แต่ละข้อจ่ายไปแล้วหนึ่งรอบวัด:**

1. ⚠️ **Google tag ที่มีอยู่ก่อนทำให้ทุกอย่างนับซ้ำ** — container ไหนที่เคยรัน WordPress Site Kit มีอยู่แล้ว · merge-import จะเอาของเราไปวางข้าง ๆ แล้วทุก session เป็นสองเท่า **แบบกู้คืนไม่ได้** ไม่มีทางแยกทีหลังว่าอันไหน session จริง · เช็คก่อนทำอย่างอื่นทั้งหมด
2. ⚠️ **ใช้ `eventSettingsTable` ไม่ใช่ `configSettingsTable`** — ตัวหลังดูน่าเชื่อพอกันใน JSON แต่ถือ gtag *config* และ **ทิ้งชื่อ parameter ที่ไม่รู้จักเงียบ ๆ**
3. ⚠️ **`gtm.js` ถูก cache 15 นาที** — การแก้ที่ถูกต้องจะดูเหมือนพัง · ตรวจกับ `gtm.js` ที่เสิร์ฟจริง อย่าตรวจจาก UI
4. ⚠️ **GA4 แปลงสตริงที่หน้าตาเป็นตัวเลข** — `"2.10"` กลายเป็น `2.1` แล้วรวมร่างกับ section 2.1 · มองไม่เห็นทั้งใน dataLayer ใน `/g/collect` และใน GA4 UI เห็นได้เฉพาะใน BigQuery export ดิบ · ใส่ prefix ให้ค่าแบบนี้ (`S3.7`)
5. ⚠️ **การกดซ้ำไม่ใช่ conversion ที่สอง** — 15% ของ bond ชุดแรกคือคนเดิมกดซ้ำ แท็บเล็ตเครื่องหนึ่งผลิต 7 bond ใน 20 วินาที · กดทิ้งตั้งแต่ต้นทาง เพราะคลิกเดียวป้อนสอง sink การแก้ฝั่งรายงานต้องเขียนสองครั้ง · **key ที่ page + channel + cta_id**: sticky CTA ที่ใช้ร่วมพก id เดียวกันทุกหน้า การ key ที่ id อย่างเดียวจะทิ้งคนที่แปลงผลจากสองหน้าบริการคนละหน้า ซึ่งพอดีเป็นคลิกที่พก product line ที่สองมา
6. ⚠️ **แถวทดสอบที่ดูเหมือนของจริงคืออันที่อันตราย** — คลิกตรวจสอบสามครั้งพก `cta_id` จริงบนหน้าจริง มองด้วยตาไม่เห็น · จับได้เพราะมันเป็น bond `paid_search` เพียงชุดเดียวบนแบรนด์ที่ไม่ได้ยิงโฆษณา *และ* เป็น bond ชุดเดียวที่มี `product_line` · ข้อตกลง: คลิกตรวจสอบทุกครั้งพก `utm_campaign` ขึ้นต้น `p<phase>-` และลบทิ้งวันเดียวกัน
7. ⚠️ **ผลจาก GA4 Data API ของวันเดียวกันยังไม่นิ่ง** — อ่านสด วันหนึ่งได้ 24 conversion พอนิ่งแล้วเป็น 26 · **มีการ revert การเปลี่ยนแปลงที่ทำงานได้อยู่แล้วเพราะความต่างนี้** · **ห้าม reconcile วันที่อายุน้อยกว่า 24 ชั่วโมง** และอ่านซ้ำวันถัดไปทุกครั้งที่ตัวเลขไม่ตรงก่อนสรุป
8. ⚠️ **ตัวควบคุมที่ต่างจากตัวทดสอบมากกว่าหนึ่งตัวแปร ไม่ใช่ตัวควบคุม** — วันควบคุมข้างบนตรงเป๊ะ และมันอายุสองวันขณะที่วันทดสอบอายุไม่กี่ชั่วโมง · **อายุของข้อมูลเป็นตัวแปร** พลาดง่ายเพราะมันเป็นคุณสมบัติของ *ตอนที่เราไปดู* ไม่ใช่ของสิ่งที่ทดสอบ
9. ⚠️ **การตั้งเกณฑ์ล่วงหน้ากันการอ่านตัวเลขเข้าข้างตัวเอง ไม่ได้กันตัวเลขผิด** — เกณฑ์ในข้อ 7 ตั้งไว้ล่วงหน้า ทำตามเป๊ะ และยังสรุปผิดอยู่ดี
10. ⚠️ **นับ device ไม่ใช่นับแถว** — อัตรา conversion ไร้คุกกี้ดูเหมือน regression 4 เท่าที่ระดับแถว และเป็นความต่างของ device เดียวที่ระดับ device · การกดรัวจากคนเดียวทำให้ทุกอัตราที่คิดจากแถวพองหมด

- 📌 **นิยาม "deploy ที่แตะชั้น CTA" ให้ชัดทั้งสองทาง** — อ่านว่า "deploy อะไรก็ตาม" ไซต์ที่พัฒนาอยู่จริงจะไม่มีวันถึงเงื่อนไขออก และกฎที่ไม่มีใครทำตามได้จะถูกทิ้งแทนที่จะถูกทำตาม · ทดสอบที่ *บรรทัด* ที่เปลี่ยนว่ามี markup ของ CTA ไหม ไม่ใช่ที่ไฟล์ที่บังเอิญมี CTA อยู่
- 📌 **ใช้เงื่อนไขออกทันทีที่ถึง** — Deezy ถึงเมื่อ 2026-08-02 แล้วรีเซ็ตวันเดียวกันเพื่อ ship ห้ารายการ · การเลื่อนชุดที่ยังไง ๆ ก็ต้อง ship มีแต่ดันวันที่ยืนยันว่านิ่งได้ออกไปอีก · การถึงแล้วใช้เลยซื้อ baseline ที่ยืนยันว่าดีไว้ bisect ซึ่งเป็นส่วนที่มีประโยชน์จริง
- ⚠️ **สิ่งที่ระบบนี้ยังไม่ใช่:** Phase 5 (LIFF) **ยังไม่ได้สร้าง** วันนี้ระบบวัด *การกดไปทาง LINE* ไม่ใช่การเป็นเพื่อนที่ยืนยันแล้ว ใครอ่านตัวเลขนี้ต้องรู้ข้อนี้ · `value` เป็น placeholder จนกว่าแบรนด์จะเติม `tsa_product_value` ด้วยเลขจริง · ~5% ของ bond ไม่มี `ga_client_id` (คุกกี้ถูกบล็อก) join กับ GA4 ไม่ได้ — เป็นเรื่องปกติ ไม่ใช่ข้อบกพร่อง Supabase กับ GA4 จะไม่มีวันตรงกันเป๊ะ · ไม่มีข้อมูลต้นทุน จึงยังไม่มี CPA/ROAS จนกว่าจะต่อแพลตฟอร์มโฆษณา · ไม่มี identity ข้ามอุปกรณ์ คนที่ดูบนมือถือแล้วโทรจากโทรศัพท์บ้านคือสองคนสำหรับระบบนี้
- ✅ **ไม่มีอะไรต้อง migrate** — เป็นการเพิ่มล้วน แบรนด์รับไปใช้โดยทำตาม §18 · schema เป็น multi-brand อยู่แล้ว · `tsa_param`/`tsa_event` ใช้ร่วมและ **ห้าม fork รายแบรนด์** · **การรับไปใช้ไม่ต้องรอบันทึกนี้** DR ฉบับนี้ทำให้ระบบที่รันอยู่แล้วเป็นทางการ ไม่ใช่เกตอนุมัติ

**Addendum 2026-08-28 (page churn — ตัวตนของหน้าที่ขยับใต้เท้า):** operator ถามคำถามที่ถูก — ระบบวัดรับมือยังไงเมื่อ **เพิ่ม · ลบ · เปลี่ยนชื่อ · 301** หน้า คำตอบสองในสามข้อคือ "รับมือได้อยู่แล้ว" และข้อที่สามคือช่องโหว่ที่ไม่มีใครเฝ้า

**เพิ่มหน้า — ปิดแล้วโดยข้อ 10 ของ DR นี้** แถวในแผนมีอยู่ก่อนหน้าจะมี และ build ใช้สิ่งที่มัน regenerate หน้าใหม่จึงพกตัวตนมาตั้งแต่ deploy แรก ไม่มีขั้นตอนที่ต้องใช้คน

**ลบหน้า — ประวัติอยู่ครบ และเป็นเพราะออกแบบ ไม่ใช่โชค** **Layer A ถูก denormalise ลงบน bond**: `cluster_id` · `product_line` · `primary_entity` · `page_type` · `page_path` ถูก**คัดลอกลงแถวตอนคลิก** ไม่ได้ lookup ตอนอ่าน ลบหน้าพรุ่งนี้ conversion ทุกครั้งที่มันเคยสร้างยังรายงานได้ครบ ที่ขาดคือแค่ JOIN กลับไปหาแผนปัจจุบัน **นี่คือเหตุผลที่ Layer A ต้อง denormalise และห้ามเปลี่ยนเป็น lookup ตอนอ่านเพื่อความ "สะอาด"** — การทำให้ normalise คือการลบประวัติของทุกหน้าที่ถูกลบ ย้อนหลังทั้งหมด

**⚠️ เคสที่ไม่มีใครเฝ้า ไม่ใช่สองข้อข้างบน** หน้ายังอยู่ · URL เดิม · ทราฟฟิกเดิม · แต่ **`page_fingerprint` เปลี่ยน** เพราะรื้อผังแล้วย้ายหน้าข้ามหมวด · Deezy พบของจริง: `/braces-cost/` เสิร์ฟ 200 อยู่ ขณะที่ตัวตนย้ายจาก `deezy-3.7.25` (§3.7 จัดฟัน) ไป `deezy-8.6.4` (§8.6 ราคา) · bond จาก 2026-08-11 ยังถือเลขเก่าซึ่งไม่ resolve แล้ว — 1 จาก 804

**ร้ายกว่าการลบ** เพราะหน้ายังหาเงินต่อภายใต้เลขใหม่ ประวัติหน้าเดียวจึงแตกเป็นสอง fingerprint และรายงานที่ join กับแผนทิ้งครึ่งเก่าไปเงียบ ๆ · เกตเดิมมองไม่เห็นเพราะมันถามว่า `page_fingerprint IS NULL` ไม่ได้ถามว่า**ค่านั้นยัง resolve ไหม** — เป็นคำถามคนละข้อ และข้อที่สองไม่เคยถูกถาม

**กฎ:** **`page_fingerprint` ต้องถือว่าเป็นคีย์นิ่ง** การรื้อผังที่ย้ายหน้าข้ามหมวดเป็น **เหตุการณ์ด้านการวัด** ไม่ใช่แค่งานจัดระเบียบ ต้องรู้ตัวตอนทำ · เมื่อจำเป็นต้องเปลี่ยนจริง **ห้ามแก้ `tsa_bond`** — แถวนั้นคือบันทึกว่าอะไรจริง ณ วินาทีที่คลิก แก้ที่รายงานแทน

**ตัวตรวจ (ต้องรับไปด้วยทุกแบรนด์):** `daily-check.sql` **บล็อก 10** — bond ที่ `page_fingerprint` ไม่ resolve กับ `page_master` ปัจจุบัน ปกติต้องเป็น 0 · **บล็อก 11** — หนึ่ง fingerprint หลาย `page_path` ซึ่งเป็น **locale ไม่ใช่ bug** (`deezy-1` คือทั้ง `/` และ `/en/`) และระบุว่า group แบบไหนตอบคำถามอะไร: group ด้วย fingerprint = ทุกภาษารวมกัน ("คอนเทนต์ไหนแปลงผล") · group ด้วย path = แยกภาษา ("URL ไหนแปลงผล") · เลือกผิดจะได้คำตอบที่มั่นใจของคำถามที่ไม่มีใครถาม · slug rename จริงก็โผล่ที่บล็อก 11 เหมือนกัน — fingerprint เดียว สอง path ที่ไม่ใช่คู่ locale ของกันและกัน คือรูปทรงที่ต้องมองหา

**"ทำไมไม่ใช้ `fingerprint` (hash) ที่นิ่งกว่าแทน" — ถามแล้ว ตอบแล้ว และเป็นคำตอบระดับ federation:** `fingerprint` คือ id ของ **แถว** ตามธรรมเนียม EYWA มีอยู่ทุกตาราง (`brands` · `seo_citations` · `seo_branches` · `logs_2026` · …) คู่กับ `fingerprint_display_name` — ไม่ใช่ตัวตนของหน้า · ตรวจทั้ง schema แล้ว **ทุกตารางที่ชี้มาที่หน้าใช้ `page_fingerprint` / `*_page_fp` ทั้งหมด ไม่มีข้อยกเว้น**: `from_page_fp`/`to_page_fp` (internal links) · `ad_landing_page_fp` (keywords) · `page_fp` (citations, editorial reviews) · `cited_page_fp` (LLM citations) · `link_target_page_fp` (brand mentions) · `optimized_for_page_fp` (voice search) · `parent_page_fp` (page master เอง) · การสลับ `tsa_bond` ไปใช้ hash จะทำให้ bond เป็นสิ่งเดียวใน federation ที่ระบุตัวหน้าไม่เหมือนใคร และ join กับตารางเหล่านั้นตรง ๆ ไม่ได้อีก ต้องกระโดดผ่าน `page_master` ทุกครั้ง · เชิงการอ่านก็แพ้: `deezy-8.6.4` บอกหมวดได้ทันทีในรายงาน GA4 ส่วน `page_63069EDB76834DDA` ไม่บอกอะไร (และ prefix แบรนด์จำเป็นอยู่แล้ว — GA4 แปลงสตริงหน้าตาเป็นตัวเลข ดูกับดักข้อ 4)

**ถ้าวันหนึ่งต้องการความนิ่งจริง ทางที่ถูกไม่ใช่สลับ id** แต่คือ **alias table `retired_page_fp -> current_page_fp`** ให้รายงาน join ผ่าน — ถูกกว่า แก้ประวัติเก่าย้อนหลังได้ และไม่แตะทั้ง `tsa_bond` และธรรมเนียมของ federation · เกณฑ์ที่ Deezy ตั้งไว้: ทำเมื่อบล็อก 10 ขึ้นเลขสองหลัก ที่ 1 ใน 804 ยังไม่คุ้ม

**References:** DR-055 (CTA attribution — ชั้นบังคับ) · DR-056 (web performance — จังหวะโหลดแท็ก) · DR-046 (shared-table governance) · SOP + install checklist §18 ที่ `eywa-deezy/docs/analytics/measurement-architecture.md` · จุดเข้า `eywa-deezy/docs/analytics/README.md` · เกตประจำวัน `eywa-deezy/docs/analytics/daily-check.sql` · migration `eywa-deezy/deployment/supabase-load/38`–`44` · staging draft `eywa-deezy/docs/proposed-protocol-changes/DR-054-eywa-tsaheylu-measurement-system.md`
...
- ✅ **Smile Scape รันแล้ว 2026-08-09** — ชั้นที่ 1 หัวบริการหายจากคลัง **8 คำ** (`ครอบฟัน` · `ฟอกสีฟัน` · `ฟันปลอม` · `สะพานฟัน` · `รักษาคลองรากฟัน` · `เกลารากฟัน` · `ดมยาสลบทำฟัน` · `รากเทียมทั้งปาก`) และ **หัวบริการ 12 คำอยู่ผิดหน้า** — รูปแบบที่พบซ้ำคือ **§5 ถือหัวบริการของ §3** (`ถอนฟัน` อยู่ hub หลังทำหัตถการ · `ผ่าฟันคุด` อยู่หน้า pericoronitis · `รักษาโรคเหงือก` อยู่หน้าโรคปริทันต์ขั้นรุนแรง) และ **หัวคำกว้างอยู่หน้าลูก** (`จัดฟัน` อยู่หน้าจัดฟันเหล็ก · `อุดฟัน` อยู่หน้าอุดฟันน้ำนมเด็ก = ผิด Q5 ด้วย) · ชั้นที่ 2 ได้ 21 hit เป็นการละเมิดจริง 12 · แก้ครบ + มินต์หัวบริการ 10 คำ (`content-plan/smile-scape-wave16-2026-08-06.md` §wave16c)
- 📌 Deezy ยังไม่เคยรันตัวตรวจนี้
...
**Status:** **🔒 Locked 2026-08-03** — operator-directed หลังยุบคลัสเตอร์เหงือกของ VTH

**Scope:** **UNIVERSAL** — ทุกแบรนด์ที่ใช้ `seo_topic_cluster_master` · `seo_citations` · `seo_entity_graph` และตารางแชร์อื่นที่มี `brand_scope`

**Context:**

`periodontal-gum` (VTH, 27 หน้า) กับ `periodontics-gum` (Deezy, 47 หน้า) อธิบายหัวข้อเดียวกัน และแยกไม่ออกจริง ๆ — หน้า VTH ที่อยู่คนละชื่อใช้ `page_type` เดียวกัน parent เดียวกัน (`vth-5.3`) และ node ติดกัน (`gingivitis` 5.3.1 อยู่ข้าง `periodontitis` 5.3.2 คนละคลัสเตอร์) ส่วน entity `loose-tooth` ถูกแยกไปคนละฝั่ง

สมมติฐาน "periodontal = โรค / periodontics = สาขาวิชา" ตายเพราะข้อมูล — โรคอยู่ทั้งสองฝั่ง และรอบวางแผนเดียวกัน (2026-07-27) ผลิตหน้าออกมา**ทั้งสองชื่อ**

ทั้งสี่แถวในพื้นที่นี้มี `brand_scope ['*']` ซึ่งแปลว่าไม่ใช่คำถามระดับแบรนด์ แต่ไม่เคยมีกฎว่าเจอแบบนี้แล้วทำอะไร `Schema_Overview` บันทึกว่าตารางมีฟิลด์อะไร ไม่ได้บอกว่าใครเป็นเจ้าของชื่อ

**Decision:**

1. **`brand_scope ['*']` = แชร์ทั้งตาราง** แถวนั้นไม่ใช่ของแบรนด์ที่สร้างมัน ใครก็เสนอยุบได้ และทุกแบรนด์ได้ประโยชน์พร้อมกัน
2. **แถวที่รอด = แถวที่มาจากแบรนด์ที่ไปไกลที่สุด** ตัดสินด้วย `load_from` **ไม่ใช่ด้วยจำนวนหน้า** — แบรนด์ที่นำหน้ามีงานปลายน้ำวางอยู่บนชื่อของมันมากที่สุด
3. **ชื่อที่ปลดระวางไม่หาย** เก็บเป็น `aliases.merged_from` บนแถวที่รอด · แถวเดิม `status='merged'` พร้อมเหตุผล **ห้ามลบ**
4. **ลำดับงาน Deezy → VTH → smile-scape** แบรนด์ที่ตามหลัง dedupe คลัสเตอร์ตัวเองกับแบรนด์ที่นำ **ก่อน**เริ่มใช้

🔴 **ก่อนยุบ ตรวจข้อมูลบนแถวที่กำลังจะแพ้เสมอ** — รอบนี้แถวที่ปลดระวางถือ `descriptions` ทั้ง en+th อยู่แถวเดียว ส่วนแถวที่รอดไม่มีเลย ยุบตรง ๆ จะทำลาย prose ชุดเดียวที่มีแบบเงียบ ๆ

**Rationale:**

ร่างแรกของข้อเสนอนี้กลับทิศ — ใช้จำนวนหน้าเป็นเกณฑ์ ซึ่งจะดึง 47 หน้าของ Deezy มาอยู่บนชื่อของ VTH `periodontics-gum` ชนะเพราะเป็นแถวเดียวใน 4 แถวที่มี `load_from='deezy-dental'` · เก่าที่สุด (2026-06-06) · และเป็นแถวเดียวที่มี `canonical_names` เติมแล้ว
...
- 📌 smile-scape: `gum-soft-tissue` (26) + ลูก `periodontics-perio-disease` (39) เป็นหัวข้อเดียวกันอีก แต่เป็นโครงสองชั้นที่ Deezy ไม่มี — ยุบตรง ๆ จะทิ้งการแบ่งชั้น ตัดสินตอนถึงคิว ไม่ใช่ผลพลอยได้
- 📌 `seo_website_page_master.status` ปิดโดเมนแล้วด้วย `chk_page_status` (`Planned|Live|Merged|Dropped`) — ค่าที่พิมพ์ผิดทำให้แถวหลุดจาก `stamp-live.mjs` ถาวร (ขยาย P13)

**References:**

- `Pamrel_Content_Writing_SOP_v1_0.md` v1.3 — P15 (กฎนี้) · P5 ขยาย · P13 ขยาย
- `Schema_Overview_EYWA_v1_23.md` §`seo_topic_cluster_master`
- VTH `docs/decision-records.md` DR-VTH-008/009/010 — บันทึกฝั่งแบรนด์
- DR-045 (PAMREL)
