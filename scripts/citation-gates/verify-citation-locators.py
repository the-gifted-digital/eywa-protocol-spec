    ...
    for fp, pmid, doi, title in rows:
        ...
        if pmid and pmid.isdigit():
            ...
            if rec:
                ...
                # Two different absences, kept apart. UNDERTAGGED means MEDLINE said
                # "Review" and nothing more precise, which is the case DR-063 is about:
                # the tier has to come from reading the abstract. UNCLASSIFIED means the
                # record carries no usable type at all. Collapsing them would hide the
                # one that has a known next step behind the one that does not.
                pts = set(rec["pubtypes"])
                tier = ("T%d/%s" % (t, name) if t
                        else "PUBTYPE_UNDERTAGGED"
                        if pts & {"Review", "Introductory Journal Article"}
                        else "UNCLASSIFIED")
